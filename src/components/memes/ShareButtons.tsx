...
export const ShareButtons: React.FC<ShareButtonsProps> = ({
  ...
  const handleShare = async (platform: string) => {
    ...
    const shareText = memeData 
      ? `Engineering Pain Level: ${memeData.painScore}/10 😅 Created with EngineerMemes.AI`
      : 'Check out this engineering meme! 😅 Created with EngineerMemes.AI';
  ...
  const handleDownloadCustom = (format: 'png' | 'jpg' | 'webp') => {
    ...
    img.onload = () => {
      ...
      link.download = `engineering-meme-${Date.now()}.${format}`;
      link.href = dataUrl;
      link.click();
    };
