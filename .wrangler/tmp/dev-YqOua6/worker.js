...
// .wrangler/tmp/bundle-QNWFOe/checked-fetch.js
...
async function generateRoomCode(env, maxAttempts = 10) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      const roomId = env.ROOMS.idFromName(roomCode);
      const roomObj = env.ROOMS.get(roomId);
      const testResponse = await roomObj.fetch(new Request("https://dummy/probe", {
        method: "GET"
      }));
...
// .wrangler/tmp/bundle-QNWFOe/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-QNWFOe/middleware-loader.entry.ts
